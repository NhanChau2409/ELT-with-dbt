select
    ...
    f.value:movement.airport.iata::string as to_airport_iata,
    f.value:movement.airport.icao::string as to_airport_icao,
    f.value:movement.airport.name::string as to_airport_name,
    f.value:movement.airport.timezone::string as to_airport_tz,
...
from {{ source("S3", "S3") }} s, lateral flatten(input => s.response, path => 'departures') f
